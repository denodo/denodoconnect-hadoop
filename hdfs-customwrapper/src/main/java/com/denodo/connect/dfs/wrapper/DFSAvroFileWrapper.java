...
import com.denodo.connect.dfs.reader.DFSAvroFileReader;
...
/**
 * DFS file custom wrapper for reading Avro files stored in a DFS (Distributed File System: hdfs, S3).
 * <p>
 *
 ...
 *
 */
public class DFSAvroFileWrapper extends AbstractSecureWrapper {

    private static final  Logger LOG = LoggerFactory.getLogger(DFSAvroFileWrapper.class);
    ...
    public CustomWrapperSchemaParameter[] doGetSchemaParameters(final Map<String, String> inputValues)
        ...
        try {
            ...
            final SchemaElement javaSchema = DFSAvroFileReader.getSchema(avroSchema);
    ...
    public void doRun(final CustomWrapperConditionHolder condition, final List<CustomWrapperFieldExpression> projectedFields,
        ...
        try {
            ...
            final Schema avroSchema = AvroSchemaUtils.buildSchema(inputValues, conf);
            reader = new DFSAvroFileReader(conf, path, fileNamePattern, avroSchema, null, includePathColumn);
