...
/**
 * HDFS File Connector Custom Wrapper for reading Sequence files stored in HDFS
 ...
 */
public class HdfsSequenceFileWrapper extends AbstractCustomWrapper {
    private static final Logger logger = Logger.getLogger(HdfsSequenceFileWrapper.class);
    ...
    private static final CustomWrapperInputParameter[] INPUT_PARAMETERS = new CustomWrapperInputParameter[] {
            ...
            new CustomWrapperInputParameter(ParameterNaming.HADOOP_KEY_CLASS, "Hadoop Key class", true,
                    CustomWrapperInputParameterTypeFactory.stringType()),
            new CustomWrapperInputParameter(ParameterNaming.HADOOP_VALUE_CLASS, "Hadoop Key class", true,
                    CustomWrapperInputParameterTypeFactory.stringType()),
    ...
    public CustomWrapperSchemaParameter[] getSchemaParameters(Map<String, String> inputValues) throws CustomWrapperException {
        ...
        this.schema = new CustomWrapperSchemaParameter[] {
                new CustomWrapperSchemaParameter(ParameterNaming.HADOOP_KEY, keyType, null, !isSearchable,
                        CustomWrapperSchemaParameter.NOT_SORTABLE, !isUpdeatable, isNullable, !isMandatory),
                new CustomWrapperSchemaParameter(ParameterNaming.HADOOP_VALUE, valueType, null, !isSearchable,
                        CustomWrapperSchemaParameter.NOT_SORTABLE, !isUpdeatable, isNullable, !isMandatory) };
        return this.schema;
    ...
    public void run(CustomWrapperConditionHolder condition, List<CustomWrapperFieldExpression> projectedFields, CustomWrapperResult result,
        ...
        if (logger.isDebugEnabled()) {
            ...
            logger.debug("HADOOP_KEY_CLASS: " + inputValues.get(ParameterNaming.HADOOP_KEY_CLASS)); //$NON-NLS-1$
            logger.debug("HADOOP_VALUE_CLASS: " + inputValues.get(ParameterNaming.HADOOP_VALUE_CLASS)); //$NON-NLS-1$
